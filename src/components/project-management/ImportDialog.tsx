...
interface ImportDialogProps {
  ...
  file: File;
  onClose: () => void;
  onSubmit: (mappings: { lat: string; lon: string; alt?: string }) => void;
}
...
const ImportDialog: React.FC<ImportDialogProps> = ({ isOpen, file, onClose, onSubmit }) => {
  ...
  const autoDetectColumns = useCallback((detectedHeaders: string[]) => {
    ...
    setMappings({ lat: autoLat, lon: autoLon, alt: autoAlt });
  }, []);
  ...
  const handleSubmit = () => {
    ...
    setError('');
    onSubmit({ lat: mappings.lat, lon: mappings.lon, alt: mappings.alt });
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Confirme ou selecione as colunas para latitude, longitude e altitude.
          </DialogDescription>
        </DialogHeader>
